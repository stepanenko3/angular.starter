...
export class TabsetComponent implements AfterContentInit {
    ...
    private _tabHeaders: QueryList<TabHeaderDirective>;

    @ContentChildren(TabContentDirective)
    private _tabContents: QueryList<TabContentDirective>;
    ...
    private loadTabs(): void {
        ...
        this._tabHeaders
            ...
            .forEach(tH => {
                const content = this._tabContents.find(tC => tC.appTabContent === tH.appTabHeader);

                if (!content) {
                    // Error if an associated tab content cannot be found for the given header.
                    throw new Error('A [TabHeader] must have a related [TabContent].');
                }
